<template>
  <div class="tk-meta-input">
    <el-input
      ...
      :type="metaInput.type"
      :placeholder="
        requiredFields[metaInput.key]
          ? t('META_INPUT_REQUIRED')
          : t('META_INPUT_NOT_REQUIRED')
      "
...
import { isQQ, t, getCookie } from "../../utils";
...
export default {
  ...
  data() {
    return {
      metaInputs: [
        {
          key: "nick",
          locale: t("META_INPUT_NICK"),
          name: "nick",
          type: "text",
        },
        {
          key: "mail",
          locale: t("META_INPUT_MAIL"),
          name: "mail",
          type: "email",
        },
        {
          key: "link",
          locale: t("META_INPUT_LINK"),
          name: "link",
          type: "text",
        },
      ],
  ...
  computed: {
    displayedFields() {
      ...
      return {
        nick: displayedFieldsSetting
          ? displayedFieldsSetting.indexOf("nick") !== -1
          : true,
        mail: displayedFieldsSetting
          ? displayedFieldsSetting.indexOf("mail") !== -1
          : true,
        link: displayedFieldsSetting
          ? displayedFieldsSetting.indexOf("link") !== -1
          : true,
      };
    },
    displayedInputs() {
    ...
    requiredFields() {
      ...
      return {
        nick: requiredFieldsSetting
          ? requiredFieldsSetting.indexOf("nick") !== -1
          : true,
        mail: requiredFieldsSetting
          ? requiredFieldsSetting.indexOf("mail") !== -1
          : true,
        link: requiredFieldsSetting
          ? requiredFieldsSetting.indexOf("link") !== -1
          : false,
      };
    },
  },
  methods: {
    ...
    initMeta() {
      ...
      const userInfo =
        userInfoStr && JSON.parse(decodeURIComponent(userInfoStr));

      if (mStr) {
        const metaData = JSON.parse(mStr);
        this.metaData.nick = metaData.nick;
        this.metaData.mail = metaData.mail || userInfo.email || null;
    ...
    checkQQ() {
      if (isQQ(this.metaData.nick)) {
        ...
        // 2. 自动填充 QQ 昵称到 [昵称]
        // 3. 自动显示 QQ 头像
        const qqNum = this.metaData.nick.replace(/@qq.com/gi, "");
