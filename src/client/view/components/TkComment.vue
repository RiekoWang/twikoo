<template>
  ...
  >
    ...
    <div class="tk-main">
      <div class="tk-row">
        <div class="tk-meta">
          <strong class="tk-nick" v-if="!convertedLink">{{
            comment.nick
          }}</strong>
          <a
          ...
          <span class="tk-tag tk-tag-red" v-if="comment.top">{{
            t("COMMENT_TOP_TAG")
          }}</span>
          <span class="tk-tag tk-tag-yellow" v-if="comment.isSpam">{{
            t("COMMENT_REVIEWING_TAG")
          }}</span>
          <small class="tk-time">
            <time :datetime="jsonTimestamp" :title="localeTime">{{
              displayCreated
            }}</time>
          </small>
          <small class="tk-actions" v-if="isLogin">
            <a
              href="#"
              v-if="comment.isSpam"
              @click="handleSpam(false, $event)"
              >{{ t("ADMIN_COMMENT_SHOW") }}</a
            >
            <a
              href="#"
              v-if="!comment.isSpam"
              @click="handleSpam(true, $event)"
              >{{ t("ADMIN_COMMENT_HIDE") }}</a
            >
            <a
              href="#"
              v-if="!comment.rid && comment.top"
              @click="handleTop(false, $event)"
              >{{ t("ADMIN_COMMENT_UNTOP") }}</a
            >
            <a
              href="#"
              v-if="!comment.rid && !comment.top"
              @click="handleTop(true, $event)"
              >{{ t("ADMIN_COMMENT_TOP") }}</a
            >
          </small>
        </div>
        <tk-action
      ...
      <div
        class="tk-content"
        :class="{
          'tk-content-expand': isContentExpanded || !showContentExpand,
        }"
        ref="tk-content"
      >
        <span v-if="comment.pid"
          >{{ t("COMMENT_REPLIED") }}
          <a class="tk-ruser" :href="`#${comment.pid}`">@{{ comment.ruser }}</a>
          :</span
        >
        <span
      ...
      <div
        class="tk-extras"
        v-if="comment.ipRegion || comment.os || comment.browser"
      >
  ...
  methods: {
    ...
    async checkAuth() {
      ...
      if (this.$tcb) {
        ...
        this.isLogin =
          this.$twikoo.serverConfig && this.$twikoo.serverConfig.IS_ADMIN;
      }
    },
    ...
    popupLightbox(event) {
      ...
      if (
        target.tagName === "IMG" &&
        !target.classList.contains("tk-owo-emotion")
      ) {
